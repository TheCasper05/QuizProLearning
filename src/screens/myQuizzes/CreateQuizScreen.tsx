...
const CreateQuizScreen = ({ navigation }: any) => {
  const { user } = useAuth();
  ...
  // Questions
  const [questions, setQuestions] = useState<Omit<Question, 'questionId'>[]>([
    {
      question: '',
      type: 'multiple',
      options: ['', '', '', ''],
  ...
  const handleSaveQuiz = async () => {
    ...
    if (!user) {
      Alert.alert('Error', 'Debes estar autenticado para crear un quiz');
      return;
    }
    ...
    try {
      ...
      // Add questionId to questions and remove undefined fields
      ...
      // Create quiz object without undefined fields
      const quizData: any = {
        title: title.trim(),
      ...
      console.log('✅ Quiz guardado:', savedQuiz.quizId);

      // Update user stats
      await UserService.incrementQuizzesCreated(user.id);
      console.log('✅ Estadísticas actualizadas');

      // Navegar de vuelta a MyQuizzesMain (que recargará automáticamente)
      navigation.goBack();
      ...
      Alert.alert('Error', error.message || 'No se pudo crear el quiz');
    } finally {
      setLoading(false);
    }
  };
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <Text style={styles.headerTitle}>Crear Nuevo Quiz</Text>
          <Text style={styles.headerSubtitle}>
            Completa la información y agrega las preguntas
          </Text>
        </View>
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={styles.saveButtonText}>Crear Quiz</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
